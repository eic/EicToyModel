...
```
  ...
  Export a CAD file (.stp extension expected) with this single container volume. 
This method should probably be removed, see EicToyModel::Export() call.
```
...
```
  ...
  Various ways to define the length of a particular detector along the lateral direction with 
respect to the stack alignment axis. One can either define it with respect to some other 
detector in a neighboring stack or in absolute numbers. The former option is preferred, 
since the detector configuration then becomes more flexible in a sense the detectors move 
synchronously when something changes in oe of the other stacks (like its offset with respect 
to the IP). 

  how         : defines a way the crack between the detector stacks (say between e-endcap and 
barrel) gets modified; possible values are: etm::kDent, etm::kRecess and etm::kWall; try them out. 
```
